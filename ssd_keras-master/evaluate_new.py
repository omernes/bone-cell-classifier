...
def evaluate_dataset(imageset_path):
    ...
    for filename in filenames:
        ...
        if img.shape != (300, 300, 3):
            continue
            # i += 1
            # # display_image(img, gt_boxes)
            # print(filename, " :: ", img.shape)
            #
            # img, gt_boxes, inverter = resize(img, labels=gt_boxes, return_inverter=True)
            # print(filename, " :: ", img.shape)
            # display_image(img2, gt_boxes2)
        ...
        # y_pred_decoded_inv = apply_inverse_transforms([y_pred_decoded], [[inverter] if inverter else []])
        # y_pred_decoded_inv = y_pred_decoded_inv[0][0]

        # display_image(img, gt_boxes)
        # display_image(img, y_pred_decoded)
